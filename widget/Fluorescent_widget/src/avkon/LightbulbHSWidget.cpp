...
void LightbulbHSWidget::postWidget(int unreadCount, bool showGlobalUnreadCnt, bool showChatUnreadCnt)
{
  bool needToRender;
...
void LightbulbHSWidget::renderWidget() {
  ...
  fluorescent->render(painter,QRect(presencePosition,QSize(presenceSize,presenceSize)));
  ...
  if(err == KErrNone)
    {
      ...
      TApaAppInfo appInfo;
      TUid aAppUid = { 0xE00AC666 }; // UId of the application to be launched
...
void LightbulbHSWidget::loadSkin(QString path) {
  ...
  fluorescent = new QSvgRenderer(QString(":/Fluorescent"));
